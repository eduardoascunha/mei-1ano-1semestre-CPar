...
void add_source(int M, int N, int O, float *x, float *s, float dt) {
  ...
  // Paraleliza o loop que atualiza o vetor x
...
void set_bnd(int M, int N, int O, int b, float *x) {
  ...
  // Set corners (this part can't be parallelized as the updates are dependent on neighboring values)
...
void lin_solve(int M, int N, int O, int b, float *x, float *x0, float a, float c) {
    ...
    do {
        ...
        //#pragma omp parallel for simd schedule(static) private(old_x, change) reduction(max:max_c)
        #pragma omp parallel for schedule(static) private(old_x, change) reduction(max:max_c)
        for (int k = 1; k <= O; k++) {
            for (int j = 1; j <= N; j++) {
                for (int i = 1 + (j + k) % 2; i <= M; i += 2) {
                    old_x = x[IX(i, j, k)];
                    float sum_i = x[IX(i - 1, j, k)] + x[IX(i + 1, j, k)];
                    float sum_j = x[IX(i, j - 1, k)] + x[IX(i, j + 1, k)];
                    float sum_k = x[IX(i, j, k - 1)] + x[IX(i, j, k + 1)];
                    x[IX(i, j, k)] = (x0[IX(i, j, k)] + a * (sum_i + sum_j + sum_k)) / c;
